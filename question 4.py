...
def simulated_annealing_vns(file_name, max_iter, max_iter2, T0, alpha):
    ...
    for i in range(max_iter):
        ...
        for index, prob in enumerate(probabilities):
            ...
            if rnd < cumulative_probability:
                new_solution = local_search_functions[index](current_solution, distance_table)

                if new_solution:
                    delta = new_solution[0] - current_solution[0]
        ...
        temperature *= alpha  # Update the temperature
        print(f"Iteration {i + 1}: Best Cost = {best_solution[0]}, Current Cost = {current_solution[0]}, Temperature = {temperature}, Probabilities = {probabilities}")

    return best_solution

# Run Simulated Annealing VNS for TSP
T0 = 1000  # Initial temperature
alpha = 0.99  # Cooling rate
best_tsp_solution = simulated_annealing_vns("ex5.txt", 10, 50, T0, alpha)
